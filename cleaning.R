### This code preprocesses the data for the following analysis.
### The code includes imputation, variable recoding, data reduction and
### creating some new variable which is of our primary interest.

library(readr)

data <- read_csv("rawdata.csv")

### Consider rent data only
...
###nyc
data$is_nyc2 <- as.numeric((((data$Mother_Birth!=9)|(data$Father_Birth!=9))&(data$Householder_Birth==9))|(((data$Mother_Birth==9)|(data$Father_Birth==9))&(data$Householder_Birth!=9)))
data$is_nyc1 <- as.numeric((data$Mother_Birth!=9)&(data$Father_Birth!=9)&(data$Householder_Birth!=9))
...
write.csv(data,file="nycRent.csv",row.names = F)
